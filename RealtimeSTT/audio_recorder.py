...
def _ensure_file_handler():
    """
    Ensure the module logger always writes to the configured log file.
    Invoked from every process/thread entry point so multiprocessing
    children inherit the same destination.
    """
    ...
    if existing_file_handler is None:
        ...
        log_format = "%(asctime)s.%(msecs)03d - RealTimeSTT: %(name)s - %(levelname)s - %(message)s"
...
class AudioToTextRecorder:
    ...
    def _audio_data_worker(
        ...
        def initialize_audio_stream(audio_interface, sample_rate, chunk_size):
            ...
            def validate_device(device_index):
                ...
                try:
                    ...
                    # Get available channels for multi-channel support
                    max_input_channels = device_info.get('maxInputChannels', 1)
                    # Use all available channels (up to 18 for Aggregate Device compatibility)
                    test_channels = min(max_input_channels, 18) if max_input_channels > 1 else 1
            ...
            while not shutdown_event.is_set():
                try:
                    ...
                    # Use all available channels (up to 18 for Aggregate Device compatibility)
                    # This ensures we capture all mics in the Aggregate Device
                    num_channels = min(max_input_channels, 18) if max_input_channels > 1 else 1
                    
                    logger.debug(f"Opening stream with device index {input_device_index}, "
                                f"sample_rate={sample_rate}, chunk_size={chunk_size}, channels={num_channels}")
                    ...
                    logger.info(f"Microphone connected and validated (device index: {input_device_index}, "
                                f"sample rate: {sample_rate}, chunk size: {chunk_size}, channels: {num_channels})")
        ...
        def preprocess_audio(chunk, original_sample_rate, target_sample_rate, num_channels=1):
            """Preprocess audio chunk with multi-channel to mono conversion."""
            if isinstance(chunk, np.ndarray):
                # Handle multi-channel to mono conversion
                if chunk.ndim == 2:
                    # Multi-channel: mix all channels to mono
                    chunk = np.mean(chunk, axis=1)
                elif chunk.ndim > 2:
                    # Reshape and mix if needed
                    chunk = np.mean(chunk.reshape(-1, num_channels), axis=1)
                ...
                # Handle multi-channel audio (interleaved format)
                if num_channels > 1:
                    # Reshape to separate channels: [ch0, ch1, ch2, ...] -> [[ch0, ch1, ...], [ch0, ch1, ...], ...]
                    num_samples = len(chunk) // num_channels
                    if num_samples * num_channels == len(chunk):
                        chunk = chunk.reshape(num_samples, num_channels)
                        
                        # Smart channel mixing for Aggregate Device:
                        # - Channels 0-15: BlackHole (system audio) - only include if active
                        # - Channels 16-17: Microphones - ALWAYS include (guaranteed mic input)
                        # This ensures mic input is never lost while avoiding silent channel dilution
                        
                        chunk_float = chunk.astype(np.float32)
                        rms_per_channel = np.sqrt(np.mean(chunk_float ** 2, axis=0))
                        
                        # Always include mic channels (16-17) - these are your voice inputs
                        mic_channels = [16, 17] if num_channels > 17 else ([16] if num_channels > 16 else [])
                        
                        # For BlackHole channels (0-15), only include if they have activity
                        # Use a reasonable threshold to detect system audio
                        blackhole_threshold = 20.0  # RMS threshold for system audio
                        blackhole_channels = [i for i in range(min(16, num_channels)) if rms_per_channel[i] > blackhole_threshold]
                        
                        # Combine: always include mics + active BlackHole channels
                        channels_to_mix = mic_channels + blackhole_channels
                        
                        if channels_to_mix:
                            # Mix selected channels
                            channels_to_mix = [c for c in channels_to_mix if c < num_channels]  # Safety check
                            if channels_to_mix:
                                chunk = np.mean(chunk[:, channels_to_mix], axis=1).astype(np.int16)
                            else:
                                # Fallback: just use mic channels if available
                                if mic_channels and mic_channels[0] < num_channels:
                                    chunk = chunk[:, mic_channels[0]].astype(np.int16)
                                else:
                                    chunk = np.mean(chunk, axis=1).astype(np.int16)
                        else:
                            # Fallback: mix all channels if no selection worked
                            chunk = np.mean(chunk, axis=1).astype(np.int16)
                    else:
                        # If not evenly divisible, just take first channel (fallback)
                        logger.debug(f"Warning: Audio chunk length {len(chunk)} not divisible by {num_channels}, using first channel only")
                        chunk = chunk[::num_channels]
                ...
                if original_sample_rate != target_sample_rate:
                    ...
                    # Ensure int16 dtype even if no resampling
                    chunk = chunk.astype(np.int16)
        ...
        num_channels = 1  # Track number of channels for multi-channel devices
        chunk_size = 1024  # Increased chunk size for better performance

        def setup_audio():  
            nonlocal audio_interface, stream, device_sample_rate, input_device_index, num_channels
            try:
                ...
                for rate in sample_rates_to_try:
                    try:
                        ...
                        result = initialize_audio_stream(audio_interface, device_sample_rate, chunk_size)
                        if result is not None:
                            if isinstance(result, tuple):
                                stream, num_channels = result
                            else:
                                stream = result
                                num_channels = 1
                            logger.debug(
                                f"Audio recording initialized successfully at {device_sample_rate} Hz, "
                                f"reading {chunk_size} frames at a time, {num_channels} channels"
                            )
                            return True
                    except Exception as e:
        ...
        try:
            ...
            while self.is_running:
                ...
                if self.is_recording:
                    ...
                    logger.info(f"[AUDIO_DEBUG] _realtime_worker processing audio: {len(audio_array)} samples, recording={self.is_recording}")
    ...
    def _on_realtime_transcription_update(self, text):
        ...
        """
        logger.info(f"[AUDIO_DEBUG] _on_realtime_transcription_update called: text='{text[:50]}...', is_recording={self.is_recording}, callback_exists={self.on_realtime_transcription_update is not None}")
